...
<head>
    ...
    <style>
        ...
        .submenu-content a:hover {
            ...
            .modal-projects-table td:last-child {
...
<body>
        ...
        <div class="container">
            <div class="charts-grid">
                ...
                <div class="chart-container">
                    <h3 class="chart-title">📈 المشاريع حسب الأقاليم والولايات</h3>
    ...
    <script>
        ...
        function openModal(type, label, count) {
            ...
            fetch(`get_projects_details.php?type=${encodeURIComponent(type)}&value=${encodeURIComponent(label)}`)
                .then(r => r.json())
        ...
        function renderProjects(projects, body) {
            ...
            let rows = projects.map((p, i) => {
                const val = parseFloat(p.montant);
                const montant = !isNaN(val)
                    ? val.toLocaleString('fr-FR', {minimumFractionDigits: 0, maximumFractionDigits: 3}) + ' مليون دينار'
                    : '-';
                return '<tr>'
                    + '<td>' + (i + 1) + '</td>'
                    + '<td style="text-align:right">' + sujet + '</td>'
                    + '<td style="direction:ltr !important; text-align:left; unicode-bidi:embed;">' + montant + '</td>'
                    + '</tr>';
            }).join('');

            body.innerHTML = '<table class="modal-projects-table" style="direction:rtl;">'
                + '<thead><tr>'
                + '<th>#</th>'
                + '<th>موضوع المشروع</th>'
                + '<th style="direction:ltr;">المبلغ (مليون دينار)</th>'
                + '</tr></thead>'
                + '<tbody>' + rows + '</tbody>'
                + '</table>';
        }
